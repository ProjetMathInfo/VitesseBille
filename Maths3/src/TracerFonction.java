...
// valeurs par defaut
	...
	private FonctionDerivable fEnCours;
	/*-----------------------*/
	public TracerFonction(int w,int h) {
		...
		JMenu mToboggan=new JMenu("Toboggan");
		jmd.add(mToboggan);

		JMenuItem plan=new JMenuItem("Toboggan Plan");
		mToboggan.add(plan);
		JMenuItem raide=new JMenuItem("Toboggan Raide");
		mToboggan.add(raide);
		JMenuItem doux=new JMenuItem("Toboggan Doux");
		mToboggan.add(doux);
	...
	getContentPane().add(new JScrollPane(table));*/
		final JPanel panneauDeContenu = new JPanel(new BorderLayout());
		...
		compositePM.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent evt) {
				...
				FonctionDerivable f_pm=new TobogganDoux() ;
		        //FonctionDerivable f_pm=new TobogganRaide() ;
		        //FonctionDerivable f_pm=new TobogganPlan() ;
				Fonction integrale = new FonctionDeIntegrale(f_pm);
				CalculIntegrale c=new CalculIntegrale(integrale);
				//CalculIntegrale c=new CalculIntegrale(new Puissance(new Lineaire(),-.75));
				tab2=c.executerComposite(30);
				panneauDeContenu.remove(table);
				table=new JTable(new DefaultTableModel(tab2, title));
		...
		trapeze.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent evt) {
				form.setText("Composite Trapeze");
				System.out.println("Trapeze");
		        //FonctionDerivable f_tr=new TobogganRaide() ;
		        FonctionDerivable f_tr=new TobogganPlan() ;
				Fonction integrale = new FonctionDeIntegrale(f_tr);
				CalculIntegrale c=new CalculIntegrale(integrale);
				//CalculIntegrale c=new CalculIntegrale(new Puissance(new Lineaire(),-.75));
				tab2=c.executerComposite(30);
				panneauDeContenu.remove(table);
				table=new JTable(new DefaultTableModel(tab2, title));
				panelOnglet.addTab("FTrapeze", tiger,new JScrollPane(table));
				panneauDeContenu.add(panelOnglet,BorderLayout.CENTER);
				//c.setComposite(new PointDuMilieu());
				//Object[][] t=c.executer((int)TracerFonction.getIntervalle());
				texteFonction.setText(c.getFonction()+"");
		...
		x34.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent evt) {
				CalculIntegrale c=new CalculIntegrale();
				c.setFonction(new Puissance(new Produit(new Constante(4.0),new Lineaire()),1/4));
				Object[][] t=c.executer((int)TracerFonction.getIntervalle());
				//setTab(t);
				System.out.println(t);
				texteFonction.setText(c.getFonction()+"");
